...
const VIDEO_CONTROLS_HIDE_DELAY = 3000; // 3 seconds of inactivity
...
function setupEventListeners() {
  ...
  if (elements.btnToggleSidebar) {
    elements.btnToggleSidebar.addEventListener('click', () => {
      ...
      if (elements.sidebar.classList.contains('collapsed')) {
        ...
        // opened: show arrows pointing left
  ...
  if (elements.btnCloseVideo) {
    elements.btnCloseVideo.addEventListener('click', (e) => {
      e.stopPropagation();
      closeInlineVideo();
    });
  }
...
function resetVideoPlayerUI() {
  ...
  // Clicking the video area itself should not close it (defensive) and can toggle play/pause
  if (v) {
  ...
  // Initialize mute button icon based on current state
  if (muteBtn && v) {
    muteBtn.innerHTML = v.muted ? '<i class="fa-solid fa-volume-xmark"></i>' : '<i class="fa-solid fa-volume-high"></i>';
  }

  // Attach play/pause listeners for auto-show (every reset since video is new)
  if (v) {
    v.addEventListener('play', showVideoControls);
    v.addEventListener('pause', showVideoControls);
  }

  // Setup auto-hide for controls (mouse inactivity) - guarded to run once
  setupVideoControlsAutoHide();

  // Show controls when video UI is (re)built
  showVideoControls();
}
...
async function loadGameMedia(appId) {
  ...
  try {
    ...
    if (movies.length > 0 && elements.videoPlayerContainer) {
      ...
      playVideoInModal(src, preferred.name || 'Trailer', appId); // try with sound (will fallback to muted if autoplay policy blocks)
    }
  } catch (err) {
...
function playVideoInModal(videoUrl, title = 'Trailer', fallbackAppId = null, startMuted = false) {
  ...
  // Query mute button once for icon updates (bound in resetVideoPlayerUI)
  const muteBtn = elements.videoPlayerContainer.querySelector('#btnVideoMute');

  // Helper: try to play (unmuted if possible). On autoplay block, fallback to muted.
  ...
  if (isLikelyHls && window.Hls && Hls.isSupported()) {
    try {
      ...
      hls.on(Hls.Events.MANIFEST_PARSED, function () {
        ...
        if (muteBtn) {
          muteBtn.innerHTML = videoEl.muted ? '<i class="fa-solid fa-volume-xmark"></i>' : '<i class="fa-solid fa-volume-high"></i>';
        }
        attemptPlay();
      });
...
// --- Video controls auto-hide (fade after inactivity, show on mouse/controller use) ---
function showVideoControls() {
  if (!elements.videoPlayerContainer) return;
  elements.videoPlayerContainer.classList.add('show-controls');
  // Note: do NOT remove 'hide-cursor' here. Cursor hide is only cleared on actual mouse movement.
  // This keeps cursor hidden when showing controls via controller or autoplay.
  resetVideoControlsHideTimer();
}

function hideVideoControls() {
  if (!elements.videoPlayerContainer) return;
  elements.videoPlayerContainer.classList.remove('show-controls');
  // Hide the mouse cursor (only while playing) so it isn't in the way of the video
  // when controls have faded. Reappears on mouse move or controller activity.
  const v = elements.videoPlayerContainer.querySelector('#detailVideoEl');
  if (v && !v.paused) {
    elements.videoPlayerContainer.classList.add('hide-cursor');
  }
}

function resetVideoControlsHideTimer() {
  clearTimeout(videoControlsHideTimer);
  videoControlsHideTimer = setTimeout(hideVideoControls, VIDEO_CONTROLS_HIDE_DELAY);
}

function setupVideoControlsAutoHide() {
  const container = elements.videoPlayerContainer;
  if (!container || container.dataset.controlsAutoHideSetup === 'true') return;

  container.dataset.controlsAutoHideSetup = 'true';

  // Mouse moved in the window shows controls (even if cursor was already in window, only on actual move)
  // Attach to document so "moved in the window" works anywhere
  const mouseMoveHandler = () => {
    if (elements.videoPlayerContainer && !elements.videoPlayerContainer.classList.contains('hidden')) {
      // On actual mouse movement: show controls and ensure cursor is visible
      elements.videoPlayerContainer.classList.remove('hide-cursor');
      showVideoControls();
    }
  };
  document.addEventListener('mousemove', mouseMoveHandler);

  // Show on any custom control button interaction (in addition to their stopPropagation)
  const controls = container.querySelector('#customVideoControls');
  if (controls) {
    controls.addEventListener('click', showVideoControls, true); // capture to catch early
  }

  // Show initially (resetVideoPlayerUI will also call show and attach per-video listeners)
  showVideoControls();
}
